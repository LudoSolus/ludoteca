...
import type { ICreateUserRequest } from "./create-user.interface";
import type { AxiosResponse } from "axios";
import { from, type Observable } from "rxjs";

export class CreateUserCommand implements ICommand {
    constructor(private body: ICreateUserRequest) {}

    execute(handler: CommandsHandlerService): Observable<AxiosResponse<ICommandResult>> {
        const url = `${PUBLIC_API_URL}/commands/admin/users/register`;
