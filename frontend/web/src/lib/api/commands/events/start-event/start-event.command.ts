...
export class StartEventCommand implements ICommand {
	constructor(private eventId: string) {}

	execute(handler: CommandsHandlerService): Observable<AxiosResponse<ICommandResult>> {
		const url = `${PUBLIC_API_URL}/commands/admin/events/${this.eventId}/start`;
		return from(handler.axios.put<ICommandResult>(url, this));
	}
}
