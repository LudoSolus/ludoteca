...
import type { IBoardGame } from "../list-board-games/list-board-games.interface";
import { ECategory } from "$lib/shared/enums/category.enum";

export class GetRecentBoardGamesQuery implements IQuery<IBoardGame[]> {
    constructor() { }

    execute(queriesHandler: QueriesHandlerService): Observable<IQueryResult<IBoardGame[]>> {
        const url = `${PUBLIC_API_URL}/queries/users/history/recent-games`;
