<script lang="ts">
    ...
    // 🔧 Ajuste para teste:
    // let authData = $derived($token ? decodeAuthJwt($token) : null);
...
<header class="header-box flex h-15 w-full items-center justify-between px-7 py-2">
    ...
    <div>
        ...
        <!-- 🔧 Ajuste para teste:
        <ProfilePicture
            userName={authData ? authData.name : "Visitante"}
            width={profilePictureSize}
            height={profilePictureSize}
            fontSize="18px"
            onClick={toogleProfileMenuVisibility}
    ...
    {#if profileMenuIsVisible}
        ...
        <!-- 🔧 Ajuste para teste:
        {#if profileMenuIsVisible && authData}
            <div class="absolute top-15 right-3 z-12">
                <ProfileMenu
                    userPublicId={authData.publicId}
                    onLogoutClick={handleOnLogout}
        ...
        {:else if profileMenuIsVisible}
            <div class="absolute top-15 right-3 z-12 bg-white p-3 rounded shadow">
