...
<main class="flex h-full flex-col gap-10 px-5 py-7 sm:px-12 xl:px-15">
	{#if userDetails === null}
		...
		<div
			class="flex flex-col items-start justify-between gap-10 px-2 pb-20 md:flex-row md:px-10 lg:px-4"
		>
			<section class="flex w-full flex-1 flex-col items-start justify-start gap-4">
				...
				<div class="flex w-full flex-col items-start justify-start gap-3 py-5">
					{#each Array(7) as _}
						<div class="flex w-full gap-2">
							<div class="skeleton-pulse h-5 w-24 rounded bg-gray-300"></div>
							<div class="skeleton-pulse h-5 w-48 rounded bg-gray-200"></div>
						</div>
					{/each}
				</div>
			</section>
			<section class="flex h-full w-full flex-1 flex-col items-center justify-center gap-8">
				<h3 class="h3 w-full text-start">Tempo Jogado:</h3>
				<div class="flex items-center justify-center py-5">
					<div class="skeleton-pulse h-52 w-52 rounded-full bg-gray-200"></div>
				</div>
			</section>
		</div>
	{:else}
		...
		>
				...
				<div class="flex w-full flex-col items-start justify-start gap-2 py-5">
					...
					<div class="w-full lg:flex lg:gap-2">
						...
						{#if isEmailTruncate}
							...
							>
								<p
									bind:clientWidth={clientEmailWidth}
									bind:this={emailRef}
									class="cursor-pointer truncate text-start text-sm"
								>
									{userDetails.email}
								</p>
							</Tooltip>
						{:else}
							<p
								bind:clientWidth={clientEmailWidth}
								bind:this={emailRef}
								class="truncate text-start text-sm"
							>
								{userDetails.email}
							</p>
						{/if}
			...
			<section class="flex h-full w-full flex-1 flex-col items-center justify-center gap-8">
				<h3 class="h3 w-full text-start">Tempo Jogado:</h3>
				<div
					class="flex h-full max-h-90 w-full flex-1 flex-col items-center gap-2 overflow-y-auto px-1 py-2 md:px-4"
				>
